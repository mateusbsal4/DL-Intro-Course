...
class ImageFolderDataset(Dataset):
    ...
    def make_dataset(directory, class_to_idx):
        ...
        for target_class in sorted(class_to_idx.keys()):            #iterating over all directories with corresponding labels: cat:, bird:1, etc
            label = class_to_idx[target_class]
            target_dir = os.path.join(directory, target_class)
            for root, _, fnames in sorted(os.walk(target_dir)):    #iterating over all the images in a specific directory & label
    ...
    def __getitem__(self, index):
        ...
        data_dict = {"image": self.transform(self.load_image_as_numpy(self.images[index])) if self.transform != None else self.load_image_as_numpy(self.images[index]),
